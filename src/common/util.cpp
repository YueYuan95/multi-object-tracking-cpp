...
int visualize(int frame, cv::Mat img, byavs::TrackeObjectCPUs results)
{
    ...
    cv::imwrite("tracking_result_M_distance/MOT16-04/"+std::to_string(frame)+".jpg", img);
...
int writeResult(int frame, byavs::TrackeObjectCPUs tracking_results)
{
    ...
    std::ofstream outfile("tracking_result_M_distance/MOT16-04/MOT16-04.txt", std::ios::app);
