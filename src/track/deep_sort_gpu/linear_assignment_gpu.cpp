...
int LinearAssignmentGPU::min_cost_matching(DistanceMetricGPU dm, KalmanTrackerV2 kf, std::vector<cv::Rect_<float>> dt_boxes,
    std::map<int, int>& matches, std::vector<int>& um_trackers, std::vector<int>& um_detections,
    FeatureMatrix detection_feature, std::vector<int> detection_indexs,
    ...
    std::vector<int> temp_features_list;
    std::vector<int> temp_tracker_ids;

    for(int i=0; i < detection_indexs.size(); i++){
        temp_features_list.push_back(detection_indexs[i]);
    }
    ...
    dm.distance(cost_matrix, temp_features_list, detection_feature, temp_tracker_ids);
...
int LinearAssignmentGPU::matching_cascade(DistanceMetricGPU dm, KalmanTrackerV2 kf,
    ...
    //show_device_data(detection_feature, "before distance 1");
    for(int i=0; i < cascade_depth; i++){
        ...
        //show_device_data(detection_feature, "before distance 2");
        ...
        //show_device_data(detection_feature, "after distance");
...
int LinearAssignmentGPU::gate_cost_matrix(KalmanTrackerV2 kf, std::vector<std::vector<double>>& cost_matrix,
    std::vector<BaseTrackerGPU> tracker_list, std::vector<int> tracker_indexes,
...
int LinearAssignmentGPU::matching_by_iou(DistanceMetricGPU dm,
    std::map<int, int>& matches, std::vector<int>& um_detections, std::vector<int>& um_trackers,
    ...
    std::vector<BaseTrackerGPU> temp_track;
